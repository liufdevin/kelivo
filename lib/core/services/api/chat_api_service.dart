...
import 'providers/dify_chat.dart';
...
class ChatApiService {
  ...
  static Stream<StreamChunk> _sendOnce({
    ...
    if (isLocalLiteRtProvider(config)) {
      ...
      if (text.isNotEmpty) yield TextDelta(id: 'local_litert', text: text);
      yield const Finish();
      return;
    }
    if (isLocalGgufProvider(config)) {
      ...
      await for (final token in LocalGgufService.sendMessageStream(
        ...
        if (token.isNotEmpty) {
          yield TextDelta(id: 'local_gguf', text: token);
        }
      }
      yield const Finish();
      return;
